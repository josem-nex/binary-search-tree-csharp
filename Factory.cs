...
public class Factory
{
    ...
    public bool Insert(int value_find, ABB root){
        var n = Find_Node(value_find, root);
    ...
    public void Print(ABB  root, string textFormat = "0", int spacing = 2, int topMargin = 0, int leftMargin = 1)
    {
        if (root == null) return;
    ...
    public ABB Max_Value_Node(ABB root){
        if(root.Right == null) return root;
        return Max_Value_Node(root.Right);
    }
    public ABB Min_Value_Node(ABB root){
        if(root.Left == null) return root;
        return Min_Value_Node(root.Left);
    }
    public bool Delete(int value_find, ABB root){
